...
// custom sanitizer will be used instead of express-mongo-sanitize
...
const sendRateLimitEmailAlert = async (
    ip,
    limiterType,
    windowMs,
    maxRequests,
    ...
    if (now - lastAlert > 15 * 60 * 1000) {
        ...
        try {
            ...
            console.log(
                `[Rate Limit Alert] Email sent for IP: ${ip} (${limiterType})`,
            );
        } catch (err) {
            console.error(
                `[Rate Limit Alert] Failed to send email for IP ${ip}:`,
                err.message,
            );
        }
    }
...
const generalLimiter = rateLimit({
    ...
    legacyHeaders: false,
    handler: (req, res) => {
        sendRateLimitEmailAlert(
            req.ip,
            "General Limiter",
            generalLimiterDurationMinutes * 60 * 1000,
            generalLimiterMaxRequests,
        );
        res.status(429).json({
...
const authLimiter = rateLimit({
    ...
    message: {
        message: "Thử đăng nhập quá nhiều lần. Tài khoản tạm khóa 5 phut nha.",
    },
    standardHeaders: true,
    legacyHeaders: false,
    handler: (req, res) => {
        sendRateLimitEmailAlert(
            req.ip,
            "Auth Limiter",
            authLimiterDurationMinutes * 60 * 1000,
            authLimiterMaxRequests,
        );
        res.status(429).json({
            message:
                "Thử đăng nhập quá nhiều lần. Tài khoản tạm khóa 5 phut nha.",
        });
    },
});
